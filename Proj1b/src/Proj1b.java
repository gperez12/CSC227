...
public class Proj1b {
	...
	public static void main(String [] args)
	{
		...
		double age,							// Age of user on next birthday		 
			   age_delta, 					// Holds 70.0 - age and 70 - (age + 5)
			   ...
			   apr_percentage,				// Annualized percentage rate in percent
			   value_2,						// The investment worth if user waits 5.0 years in dollars 
		...
		apr_percentage = (apr*100);		//Converts APR from decimal form to percentage
		
		age_delta = 70-age;				// Subtracts 70 years minus the age of the user
										// on their next birthday
		
		value = principal * Math.pow((1+(apr/DAYS_IN_YEAR)),(DAYS_IN_YEAR * age_delta));	// Calculates value by using
																							// V = p(1 + (APR/c))^cy
		...
		age_delta = age_delta - 5;	// Calculates the years if a user decides
									// to wait 5.0 years
		
		value_2 = principal * Math.pow((1+(apr/DAYS_IN_YEAR)),(DAYS_IN_YEAR * age_delta));	// Calculates value if a user
																							// waits 5.0 extra years
		delta = value - value_2;	// Calculates the difference in dollars
